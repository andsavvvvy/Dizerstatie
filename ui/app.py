"""
Web Interface for Distributed Clustering System
"""
...
def build_sensitivity_data(local_by_node, algo_scores):
    """Build data for sensitivity analysis charts."""
    # Per-algorithm comparison across nodes
    algo_node_matrix = []
    ...
    # Silhouette distribution per algorithm
    algo_distributions = {}
...
def analysis_new(): return render_template('analysis_new.html', nodes=NODES)

@app.route('/analysis/<session_id>/monitor')
def analysis_monitor(session_id): return render_template('analysis_monitor.html', session_id=session_id)
...
def analysis_detail(session_id):
    ...
    # Build sensitivity analysis data
...
def api_system_health():
    o = check_orch(); nh = {n['name']:check_node(n['url']) for n in NODES}
    return jsonify({'overall':'healthy' if o and all(s=='healthy' for s in nh.values()) else 'degraded',
                    'orchestrator':'healthy' if o else 'unhealthy','nodes':nh})
